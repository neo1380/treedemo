...
export class TreeNode implements ITreeNode {
 ...
 get enableCheckBox(){
    return this.getDescendantField('checkBox');
  }

 get isDisabledCheckbox(){
  return this.getDescendantField('isDisabledCheckbox');
 }
  ...
  getDescendantField(key) {
    var keyName =this.options[`${key}Field`];
    if(keyName.includes('.')){
        var obj=this.data;
        var arr = this.options[`${key}Field`].split(".");
