...
export class TreeModel implements ITreeModel, OnDestroy {
 ...
 /*  private _allowCheckBox(startNode = null) {
    startNode = startNode || this.virtualRoot;
    
    if (startNode.data[this.options.checkBoxField]) {
        return startNode.data[this.options.checkBoxField]
    }

    if (startNode.children) {
      startNode.children.forEach((child) => this._allowCheckBox(child));
    }
